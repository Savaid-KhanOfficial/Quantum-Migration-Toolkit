#include "AES.hpp"
#include <openssl/evp.h>
#include <openssl/err.h>
#include <stdexcept>
#include <cstring>

using namespace std;

vector<uint8_t> AES::encrypt(const vector<uint8_t>& data, const vector<uint8_t>& key, const vector<uint8_t>& iv) {
    if (key.size() != 32) {
        throw runtime_error("AES-256 requires a 32-byte key");
    }
    if (iv.size() != 16) {
        throw runtime_error("IV must be 16 bytes");
    }
    
    // Create and initialize the context
    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
    if (!ctx) {
        throw runtime_error("Failed to create EVP_CIPHER_CTX");
    }
    
    // Initialize encryption operation with AES-256-CBC
    if (EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), nullptr, key.data(), iv.data()) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        throw runtime_error("EVP_EncryptInit_ex failed");
    }
    
    // Allocate output buffer (plaintext size + block size for padding)
    vector<uint8_t> encrypted(data.size() + EVP_CIPHER_block_size(EVP_aes_256_cbc()));
    int len = 0;
    int ciphertext_len = 0;
    
    // Encrypt the data
    if (EVP_EncryptUpdate(ctx, encrypted.data(), &len, data.data(), static_cast<int>(data.size())) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        throw runtime_error("EVP_EncryptUpdate failed");
    }
    ciphertext_len = len;
    
    // Finalize encryption (handles padding)
    if (EVP_EncryptFinal_ex(ctx, encrypted.data() + len, &len) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        throw runtime_error("EVP_EncryptFinal_ex failed");
    }
    ciphertext_len += len;
    
    // Clean up
    EVP_CIPHER_CTX_free(ctx);
    
    // Resize to actual encrypted length
    encrypted.resize(ciphertext_len);
    return encrypted;
}

vector<uint8_t> AES::decrypt(const vector<uint8_t>& data, const vector<uint8_t>& key, const vector<uint8_t>& iv) {
    if (key.size() != 32) {
        throw runtime_error("AES-256 requires a 32-byte key");
    }
    if (iv.size() != 16) {
        throw runtime_error("IV must be 16 bytes");
    }
    
    // Create and initialize the context
    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
    if (!ctx) {
        throw runtime_error("Failed to create EVP_CIPHER_CTX");
    }
    
    // Initialize decryption operation with AES-256-CBC
    if (EVP_DecryptInit_ex(ctx, EVP_aes_256_cbc(), nullptr, key.data(), iv.data()) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        throw runtime_error("EVP_DecryptInit_ex failed");
    }
    
    // Allocate output buffer
    vector<uint8_t> decrypted(data.size());
    int len = 0;
    int plaintext_len = 0;
    
    // Decrypt the data
    if (EVP_DecryptUpdate(ctx, decrypted.data(), &len, data.data(), static_cast<int>(data.size())) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        throw runtime_error("EVP_DecryptUpdate failed");
    }
    plaintext_len = len;
    
    // Finalize decryption (handles padding removal)
    if (EVP_DecryptFinal_ex(ctx, decrypted.data() + len, &len) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        throw runtime_error("EVP_DecryptFinal_ex failed - possibly incorrect key or corrupted data");
    }
    plaintext_len += len;
    
    // Clean up
    EVP_CIPHER_CTX_free(ctx);
    
    // Resize to actual decrypted length
    decrypted.resize(plaintext_len);
    return decrypted;
}

// AES S-box
const uint8_t AES::sbox[256] = {
    
};

const uint8_t AES::inv_sbox[256] = {
    
};

const uint8_t AES::rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

uint8_t AES::gf_mul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        bool hi_bit_set = (a & 0x80);
        a <<= 1;
        if (hi_bit_set) a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

void AES::add_round_key(uint8_t* state, const uint8_t* round_key) {
    for (int i = 0; i < 16; i++) {
        state[i] ^= round_key[i];
    }
}

void AES::sub_bytes(uint8_t* state) {
    for (int i = 0; i < 16; i++) {
        state[i] = sbox[state[i]];
    }
}

void AES::inv_sub_bytes(uint8_t* state) {
    for (int i = 0; i < 16; i++) {
        state[i] = inv_sbox[state[i]];
    }
}

void AES::shift_rows(uint8_t* state) {
    uint8_t temp;
    temp = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = temp;
    temp = state[2]; state[2] = state[10]; state[10] = temp;
    temp = state[6]; state[6] = state[14]; state[14] = temp;
    temp = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = state[3]; state[3] = temp;
}

void AES::inv_shift_rows(uint8_t* state) {
    uint8_t temp;
    temp = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = temp;
    temp = state[2]; state[2] = state[10]; state[10] = temp;
    temp = state[6]; state[6] = state[14]; state[14] = temp;
    temp = state[3]; state[3] = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = temp;
}

void AES::mix_columns(uint8_t* state) {
    for (int i = 0; i < 4; i++) {
        uint8_t* col = state + i * 4;
        uint8_t a[4];
        memcpy(a, col, 4);
        col[0] = gf_mul(a[0], 2) ^ gf_mul(a[1], 3) ^ a[2] ^ a[3];
        col[1] = a[0] ^ gf_mul(a[1], 2) ^ gf_mul(a[2], 3) ^ a[3];
        col[2] = a[0] ^ a[1] ^ gf_mul(a[2], 2) ^ gf_mul(a[3], 3);
        col[3] = gf_mul(a[0], 3) ^ a[1] ^ a[2] ^ gf_mul(a[3], 2);
    }
}

void AES::inv_mix_columns(uint8_t* state) {
    for (int i = 0; i < 4; i++) {
        uint8_t* col = state + i * 4;
        uint8_t a[4];
        memcpy(a, col, 4);
        col[0] = gf_mul(a[0], 14) ^ gf_mul(a[1], 11) ^ gf_mul(a[2], 13) ^ gf_mul(a[3], 9);
        col[1] = gf_mul(a[0], 9) ^ gf_mul(a[1], 14) ^ gf_mul(a[2], 11) ^ gf_mul(a[3], 13);
        col[2] = gf_mul(a[0], 13) ^ gf_mul(a[1], 9) ^ gf_mul(a[2], 14) ^ gf_mul(a[3], 11);
        col[3] = gf_mul(a[0], 11) ^ gf_mul(a[1], 13) ^ gf_mul(a[2], 9) ^ gf_mul(a[3], 14);
    }
}

void AES::key_expansion(const uint8_t* key, uint8_t* round_keys) {
    memcpy(round_keys, key, 32);
    
    for (int i = NK; i < 4 * (NR + 1); i++) {
        uint8_t temp[4];
        memcpy(temp, round_keys + (i - 1) * 4, 4);
        
        if (i % NK == 0) {
            uint8_t k = temp[0];
            temp[0] = sbox[temp[1]] ^ rcon[i / NK];
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[k];
        } else if (i % NK == 4) {
            for (int j = 0; j < 4; j++) {
                temp[j] = sbox[temp[j]];
            }
        }
        
        for (int j = 0; j < 4; j++) {
            round_keys[i * 4 + j] = round_keys[(i - NK) * 4 + j] ^ temp[j];
        }
    }
}

void AES::cipher(uint8_t* state, const uint8_t* round_keys) {
    add_round_key(state, round_keys);
    
    for (int round = 1; round < NR; round++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, round_keys + round * 16);
    }
    
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, round_keys + NR * 16);
}

void AES::inv_cipher(uint8_t* state, const uint8_t* round_keys) {
    add_round_key(state, round_keys + NR * 16);
    
    for (int round = NR - 1; round > 0; round--) {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, round_keys + round * 16);
        inv_mix_columns(state);
    }
    
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, round_keys);
}

vector<uint8_t> AES::add_padding(const vector<uint8_t>& data) {
    int padding_length = AES_BLOCK_SIZE - (data.size() % AES_BLOCK_SIZE);
    vector<uint8_t> padded = data;
    padded.resize(data.size() + padding_length, static_cast<uint8_t>(padding_length));
    return padded;
}

vector<uint8_t> AES::remove_padding(const vector<uint8_t>& data) {
    if (data.empty()) {
        throw runtime_error("Cannot remove padding from empty data");
    }
    uint8_t padding_length = data.back();
    if (padding_length > AES_BLOCK_SIZE || padding_length > data.size()) {
        throw runtime_error("Invalid padding");
    }
    vector<uint8_t> result(data.begin(), data.end() - padding_length);
    return result;
}

vector<uint8_t> AES::encrypt(const vector<uint8_t>& data, const vector<uint8_t>& key, const vector<uint8_t>& iv) {
    if (key.size() != AES_KEY_SIZE) {
        throw runtime_error("AES-256 requires a 32-byte key");
    }
    if (iv.size() != AES_BLOCK_SIZE) {
        throw runtime_error("IV must be 16 bytes");
    }
    
    // Expand the key
    uint8_t round_keys[240];
    key_expansion(key.data(), round_keys);
    
    // Add padding
    vector<uint8_t> padded_data = add_padding(data);
    vector<uint8_t> encrypted;
    encrypted.reserve(padded_data.size());
    
    // CBC mode encryption
    vector<uint8_t> prev_block = iv;
    
    for (size_t i = 0; i < padded_data.size(); i += AES_BLOCK_SIZE) {
        uint8_t block[AES_BLOCK_SIZE];
        
        // XOR with previous ciphertext block (CBC mode)
        for (int j = 0; j < AES_BLOCK_SIZE; j++) {
            block[j] = padded_data[i + j] ^ prev_block[j];
        }
        
        // Encrypt block
        cipher(block, round_keys);
        
        // Save ciphertext block
        for (int j = 0; j < AES_BLOCK_SIZE; j++) {
            encrypted.push_back(block[j]);
            prev_block[j] = block[j];
        }
    }
    
    return encrypted;
}

vector<uint8_t> AES::decrypt(const vector<uint8_t>& data, const vector<uint8_t>& key, const vector<uint8_t>& iv) {
    if (key.size() != AES_KEY_SIZE) {
        throw runtime_error("AES-256 requires a 32-byte key");
    }
    if (iv.size() != AES_BLOCK_SIZE) {
        throw runtime_error("IV must be 16 bytes");
    }
    if (data.size() % AES_BLOCK_SIZE != 0) {
        throw runtime_error("Encrypted data length must be a multiple of block size");
    }
    
    // Expand the key
    uint8_t round_keys[240];
    key_expansion(key.data(), round_keys);
    
    vector<uint8_t> decrypted;
    decrypted.reserve(data.size());
    
    // CBC mode decryption
    vector<uint8_t> prev_block = iv;
    
    for (size_t i = 0; i < data.size(); i += AES_BLOCK_SIZE) {
        uint8_t block[AES_BLOCK_SIZE];
        memcpy(block, &data[i], AES_BLOCK_SIZE);
        
        // Save current ciphertext block for next iteration
        vector<uint8_t> cipher_block(block, block + AES_BLOCK_SIZE);
        
        // Decrypt block
        inv_cipher(block, round_keys);
        
        // XOR with previous ciphertext block (CBC mode)
        for (int j = 0; j < AES_BLOCK_SIZE; j++) {
            block[j] ^= prev_block[j];
            decrypted.push_back(block[j]);
        }
        
        prev_block = cipher_block;
    }
    
    // Remove padding
    return remove_padding(decrypted);
}
