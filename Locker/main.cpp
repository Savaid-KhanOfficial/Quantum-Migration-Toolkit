...
void show_help() {
    cout << "==========================================================" << endl;
    cout << "  QUANTUM LOCKER - Command Line Tool" << endl;
    cout << "  Post-Quantum Cryptography File Protection" << endl;
    cout << "  Powered by Kyber-512 + AES-256" << endl;
    cout << "==========================================================" << endl;
    cout << "\nUSAGE:" << endl;
    ...
    cout << "        - <identity_name>.pub  (public key)" << endl;
    cout << "        - <identity_name>.priv (private key)" << endl;
    cout << "\n  encrypt <identity_name> <file_path>" << endl;
    cout << "      Encrypt a file using your identity" << endl;
    cout << "        - Loads keys from <identity_name>.pub/.priv" << endl;
    cout << "        - Encrypts <file_path> to <file_path>.qenc" << endl;
    cout << "\n  decrypt <identity_name> <file_path.qenc>" << endl;
    cout << "      Decrypt a quantum-encrypted file" << endl;
    cout << "        - Loads keys from <identity_name>.pub/.priv" << endl;
...
int cmd_genkeys(const string& identity_name) {
    try {
        ...
        cout << "  Public Key:  " << public_key.size() << " bytes" << endl;
        cout << "  Secret Key:  " << secret_key.size() << " bytes" << endl;
        
        if (!quantum.save_keys(identity_name)) {
            cerr << "[ERROR] Failed to save keys" << endl;
            return 1;
        }
        
        cout << "\n[SUCCESS] Keys generated and saved!" << endl;
        cout << "  ✓ " << identity_name << ".pub" << endl;
        cout << "  ✓ " << identity_name << ".priv" << endl;
        cout << "\nKeep your .priv file safe and never share it!" << endl;
...
int cmd_encrypt(const string& identity_name, const string& file_path) {
    try {
        ...
        if (!fs::exists(identity_name + ".pub") || !fs::exists(identity_name + ".priv")) {
            cerr << "[ERROR] Identity not found: " << identity_name << endl;
        ...
        if (!quantum.load_keys(identity_name)) {
            cerr << "[ERROR] Failed to load keys" << endl;
            return 1;
        }
        
        // Encrypt file
        string output_path = file_path + ".qenc";
        FileEncryptor::encrypt_file(quantum, file_path, output_path);
        
        cout << "\n[SUCCESS] File encrypted!" << endl;
...
int cmd_decrypt(const string& identity_name, const string& file_path) {
    try {
        ...
        // Decrypt file
        FileEncryptor::decrypt_file(quantum, file_path, output_path);
        
        cout << "\n[SUCCESS] File decrypted!" << endl;
        cout << "  Input:  " << file_path << endl;
        cout << "  Output: " << output_path << endl;
